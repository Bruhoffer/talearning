...
class EarningsTradingDashboard:
    ...
    def setup_ui(self):
        ...
        # Stock Strategy P/L frame
        stock_strategy_frame = ttk.LabelFrame(main_frame, text="Stock Strategy P/L (Buy/Short)", padding="5")
        stock_strategy_frame.grid(row=5, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        stock_strategy_frame.columnconfigure(1, weight=1)
        stock_strategy_frame.columnconfigure(3, weight=1)

        ttk.Label(stock_strategy_frame, text="LONG Stock P/L:").grid(row=0, column=0, padx=(0, 5))
        self.long_stock_pnl_label = ttk.Label(stock_strategy_frame, text="N/A", font=("Arial", 11, "bold"))
        self.long_stock_pnl_label.grid(row=0, column=1, sticky=tk.W, padx=(0, 20))

        ttk.Label(stock_strategy_frame, text="SHORT Stock P/L:").grid(row=0, column=2, padx=(0, 5))
        self.short_stock_pnl_label = ttk.Label(stock_strategy_frame, text="N/A", font=("Arial", 11, "bold"))
        self.short_stock_pnl_label.grid(row=0, column=3, sticky=tk.W)
        # <--- END OF NEW FRAME --->
    ...
    def clear_analysis_results(self):
        ...
        # <--- ADD THESE LINES --->
        self.long_stock_pnl_label.config(text="N/A", foreground="black")
        self.short_stock_pnl_label.config(text="N/A", foreground="black")
        # <--- END OF ADDED LINES --->
    ...
    def perform_iv_crush_analysis(self):
        ...
        # <--- ADD THIS LOGIC HERE --->
        # Calculate simple stock strategy P/L
        stock_price_change = post_stock_price - pre_stock_price
        stock_price_change_pct = (stock_price_change / pre_stock_price) * 100
        
        # P/L for holding 1 share
        long_stock_pnl = stock_price_change
        short_stock_pnl = -stock_price_change # (pre_stock_price - post_stock_price)
        
        self.log_message(f"")
        self.log_message(f"Stock-Only Strategy (1 share):")
        self.log_message(f"  Entry (pre-close): ${pre_stock_price:.2f}")
        self.log_message(f"  Exit (post-avg): ${post_stock_price:.2f}")
        self.log_message(f"  Price Change: ${stock_price_change:+.2f} ({stock_price_change_pct:+.2f}%)")
        self.log_message(f"  LONG Stock P/L: ${long_stock_pnl:+.2f}")
        self.log_message(f"  SHORT Stock P/L: ${short_stock_pnl:+.2f}")
        # <--- END OF NEW LOGIC --->
        ...
        # <--- ADD THIS UI UPDATE LOGIC HERE --->
        # Update Stock Strategy P/L displays
        long_color = "green" if long_stock_pnl > 0 else "red"
        short_color = "green" if short_stock_pnl > 0 else "red"
        self.long_stock_pnl_label.config(text=f"{long_stock_pnl:+.2f} ({stock_price_change_pct:+.1f}%)", foreground=long_color)
        self.short_stock_pnl_label.config(text=f"{short_stock_pnl:+.2f} ({-stock_price_change_pct:+.1f}%)", foreground=short_color)
        # <--- END OF NEW UI UPDATE LOGIC --->
